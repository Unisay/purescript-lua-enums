...
instance boundedEnumPair ::
  ( BoundedEnum a
  , BoundedEnum b
  ) =>
  BoundedEnum (Pair a b) where
  cardinality = GEnum.genericCardinality
  toEnum = GEnum.genericToEnum
  fromEnum = GEnum.genericFromEnum
...
testGenericEnum = do
  ...
  assert $ (pred =<< succ (bottom :: Pair Bit SimpleBounded)) == Just
    (Pair Zero A)
  ...
  assert $ (cardinality :: Cardinality (Pair Bit SimpleBounded)) == Cardinality
    8
  ...
  assert $ toEnum (fromEnum (None :: Option SimpleBounded)) == Just
    (None :: Option SimpleBounded)
  assert $ toEnum (fromEnum (Some A)) == Just (Some A)

  log "Checking product toEnum/fromEnum roundtrip"
  assert $
    let
      allPairs = enumFromTo bottom top :: Array (Pair Bit SimpleBounded)
    in
      (toEnum <<< fromEnum <$> allPairs) == (Just <$> allPairs)
